...
import {
  AccountsCard,
  NoAccountsCard,
} from "../../components/Cards/AccountsCard"
...
export const Dashboard: React.FC = () => {
          ...
          {GLOBAL_CURRENCIES.map((currency: string) => (
            <ListItem
              ...
              onClick={async () => {
                ...
                try {
                  const response: ExecutionResult<CreateAccountMutation> =
                    await createAccount({
                      variables: {
                        currency: currency,
                      },
            ...
            >
              <ListItemIcon>
                <div style={{ width: 32 }}>
                  {determineCurrencyIcon(currency)}
                </div>
              </ListItemIcon>
  ...
  const handleCreateNewCardClicked = async (
    e: MouseEvent<HTMLButtonElement>
  ): Promise<void> => {
    e.preventDefault()
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} md={12} lg={12}>
              <Paper className={chartPaper}>
                <Chart
                  currency={!!analyticsAccount ? analyticsAccount : "EUR"}
                />
              </Paper>
            </Grid>
          </Grid>
                  ...
                  <Grid key={account.id} item xs={12} md={4} lg={4}>
                    <Paper className={accountCardHeightPaper}>
                      <AccountsCard
                        ...
                        iban={account.iban}
                        onAccountClicked={(e: MouseEvent<HTMLButtonElement>) =>
                  ...
                  <Grid key={card.id} item xs={12} md={4} lg={4}>
                    <Paper className={apolloCardPaper}>
                      <ApolloCard
                        ...
                        validThru={
                          new Date(Date.parse(card.expiresIn)).getMonth() +
                          "/" +
